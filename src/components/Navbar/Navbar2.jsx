...
export default function SearchAppBar() {
  ...
  useEffect(()=> {
    ...
    const listOfNames = allUsernames.filter(name => {
      return name.name.toLowerCase().includes(search.toLowerCase())
    })
    
    setUsernames(listOfNames)
  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
      ...
      {usernames.map((username, i) => <div key={i}><Link to={`/user/${username.username}`}><button>{username.name}</button></Link></div>)}
    </Box>
  );
}